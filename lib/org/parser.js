...
Parser.prototype = {
  ...
  parseDate: function () {
    ...
    if (this.lexer.hasNext() && this.lexer.peekNextToken().type === Lexer.tokens.line) {
      this.document.date = this.createTextNode(this.lexer.getNextToken().content);
    } else {
  ...
  parseDirectiveBlock: function (directiveNode, verbatim) {
    ...
    var verbatimIndex = this.lexer.stream.lineNumber;
    
    while (this.lexer.hasNext() && verbatimIndex++ < this.lexer.stream.totalLines) {
      var nextToken = this.lexer.peekNextToken();
  ...
  interpretDirective: function (directiveNode) {
    ...
    case "title:":
      ...
      break;
    case "date:":
      this.document.date = directiveNode.directiveRawValue;
      break;
    case "author:":
