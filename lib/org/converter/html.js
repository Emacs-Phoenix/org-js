...
ConverterHTML.prototype = {
  ...
  convert: function () {
    ...
    var date = this.orgDocument.date ? this.convertNode(this.orgDocument.date) : null;
    var dateHTML = date ? this.tag("div", date) : null;
  ...
  tocToHTML: function (toc) {
    function tocToHTMLFunction(tocList) {
      ...
      return this.tag("ul", html, {class: 'outline'});
    }
  ...
  convertLink: function (node, childText, auxData) {
    ...
    } else if (/^image-url:/.test(srcParameterStripped)) {
      var imgText = this.getNodeTextContent(node);
      return this.inlineTag("img", null, {
        src: node.src.replace("image-url:", ""),
        alt: imgText,
        title: imgText
      }, auxData);
    } else {
